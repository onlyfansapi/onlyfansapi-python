...
from onlyfansapi import OnlyFansAPI, AsyncOnlyFansAPI, DefaultAioHttpClient
from onlyfansapi._utils import is_dict
...
logging.getLogger("onlyfansapi").setLevel(logging.DEBUG)
...
def client(request: FixtureRequest) -> Iterator[OnlyFansAPI]:
    strict = getattr(request, "param", True)
    if not isinstance(strict, bool):
        raise TypeError(f"Unexpected fixture parameter type {type(strict)}, expected {bool}")

    with OnlyFansAPI(base_url=base_url, api_key=api_key, _strict_response_validation=strict) as client:
        yield client


@pytest.fixture(scope="session")
async def async_client(request: FixtureRequest) -> AsyncIterator[AsyncOnlyFansAPI]:
