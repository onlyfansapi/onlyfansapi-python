...
to add `ONLYFANSAPI_API_KEY="My API Key"` to your `.env` file
...
async def main() -> None:
    whoami = await client.whoami.retrieve()
    print(whoami.api_key)
...
When the library is unable to connect to the API (for example, due to network connection problems or a timeout), a subclass of `onlyfansapi.APIConnectionError` is raised.

When the API returns a non-success status code (that is, 4xx or 5xx
response), a subclass of `onlyfansapi.APIStatusError` is raised, containing `status_code` and `response` properties.

All errors inherit from `onlyfansapi.APIError`.
...
except onlyfansapi.APIConnectionError as e:
    print("The server could not be reached")
    print(e.__cause__)  # an underlying Exception, likely raised within httpx.
except onlyfansapi.RateLimitError as e:
    print("A 429 status code was received; we should back off a bit.")
except onlyfansapi.APIStatusError as e:
...
client.with_options(max_retries=5).whoami.retrieve()
```
...
client.with_options(timeout=5.0).whoami.retrieve()
```
...
whoami = response.parse()  # get the object that `whoami.retrieve()` would have returned
print(whoami.api_key)
```
